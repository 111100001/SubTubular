...
module Cache =
    ...
    let private deletableFile label (file: FileInfo) =
        (HStack(5) {
            (displayWithMb $"{label}" file.Length)
                .tappable(OpenFile file, "open this file")
                .centerVertical ()

            Button(Icon.trash, RemoveFiles [| file |]).tooltip ("delete this file")
        })
    ...
    let private scopeSearch prefix (file: FileInfo) =
        ...
        (deletableFile label file).card().margin (5)
        ...
        VStack() {
            ...
            let displayPlaylistGroup (group: PlaylistGroup) =
                (VStack() {
                    ...
                    (deletableFile ("info and contents" + Icon.playlistLike) group.File).right ()

                    if group.Thumbnail <> null then
                        (deletableFile ("thumbnail" + Icon.thumbnail) group.Thumbnail).right ()
            ...
            | Some files ->
                ...
                ItemsControl(files.Other, fun f -> (deletableFile f.Name f).card().margin (5))
                    .itemsPanel (HWrapEmpty())
            | None -> header "loading..."
    ...
    let view model =
        ScrollViewer(
            ...
            Grid(coldefs = [ Star; gap; Auto ], rowdefs = [ Auto; gap; Auto; gap; Auto ]) {
                Expander("File type info " + Icon.info, fileTypes model)
