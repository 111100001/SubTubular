...
public sealed class Youtube(DataStore dataStore, VideoIndexRepository videoIndexRepo)
{
        ...
        [EnumeratorCancellation] CancellationToken cancellation = default)
    {
        ...
        Action<Exception> handleProducerError = ex =>
        {
            if (ex.GetRootCauses().OfType<InputException>().Any())
            {
                 ...
                 * an OperationCanceledException furhter up the call chain. */
                linkedTs.Cancel();
            }
        };
        ...
        PlaylistLikeScope scope, [EnumeratorCancellation] CancellationToken cancellation)
    {
        ...
        var paging = Task.Run(async () =>
        {
            ...
            finally { await changedSinceLastReturn.Writer.WriteAsync(madeChangesAfterReturningEarly); }

            // queue up writer completion unconditionally and forwarding unexpected exceptions
        }).ContinueWith(t => changedSinceLastReturn.Writer.Complete(t.Exception), cancellation);
        ...
        [EnumeratorCancellation] CancellationToken cancellation = default)
    {
        ...
        }, cancellation).WithAggregateException();
