...
public class ParallelizeTests
{
    ...
    public async Task YieldsEverythingFromAllProducers()
    {
        ...
        var parallelized = producers.Parallelize(DontHandleException, CancellationToken.None);
    ...
    public async Task YieldsImmediatelyAndHandlesCancellation()
    {
        ...
        try
        {
            await foreach (var product in producers.Parallelize(DontHandleException, cts.Token)) products.Add(product);
    ...
    void DontHandleException(Exception ex) { }
}
